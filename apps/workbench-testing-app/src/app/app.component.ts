...
import {afterEveryRender, Component, computed, DoCheck, DOCUMENT, effect, ElementRef, inject, Injector, runInInjectionContext, signal, Signal, untracked, WritableSignal} from '@angular/core';
...
export class AppComponent implements DoCheck {
  ...
  constructor() {
    ...
    if (environment.toolbarVisibility === 'always') {
      inject(WorkbenchService).settings.toolbarVisibility.set('always');
    }
  }
  ...
  private contributeWorkbenchPartToolbar(): void {
    contributeMenu('toolbar:workbench.part.toolbar', toolbar => {
      ...
      toolbar
        .addToolbarButton({icon: 'scion.expand_all', tooltip: 'Expand All', accelerator: {ctrl: true, key: '+', location: 'numpad'}, onSelect: () => console.log(`>>> Ctrl+NumPad+ [location=toolbar:workbench.part.toolbar, contributor=${part.id}]`)})
        .addToolbarButton({icon: 'scion.collapse_all', tooltip: 'Collapse All', accelerator: {ctrl: true, key: '-', location: 'numpad'}, onSelect: () => console.log(`>>> Ctrl+NumPad- [location=toolbar:workbench.part.toolbar, contributor=${part.id}]`)})
        .addToolbarMenu({icon: 'visibility'}, menu => menu
          ...
          .addMenu({icon: 'tv_options_edit_channels', label: 'Adavanced Settings', menu: {filter: true}}, menu => {
            // Add 'View in Groups' group.
            menu.addGroup({
              label: 'View in Groups',
              actions: actions => actions
                .addToolbarButton({icon: 'done_all', tooltip: 'Select All', onSelect: () => selectAll(flag6, flag7, flag8, flag9, flag10, flag11, flag12, flag13)})
                .addToolbarButton({icon: 'remove_done', tooltip: 'Deselect All', onSelect: () => deselectAll(flag6, flag7, flag8, flag9, flag10, flag11, flag12, flag13)}),
            }, group => group
            ...
            menu.addGroup({
              ...
              actions: actions => actions
                .addToolbarButton({icon: 'done_all', tooltip: 'Select All', onSelect: () => selectAll(flag14, flag15, flag16, flag17, flag18, flag19)})
                .addToolbarButton({icon: 'remove_done', tooltip: 'Deselect All', onSelect: () => deselectAll(flag14, flag15, flag16, flag17, flag18, flag19)}),
            }, group => group
            ...
            menu.addGroup({
              label: 'Node Detail',
              actions: actions => actions
                .addToolbarButton({icon: 'done_all', tooltip: 'Select All', onSelect: () => selectAll(flag20, flag21, flag22)})
                .addToolbarButton({icon: 'remove_done', tooltip: 'Deselect All', onSelect: () => deselectAll(flag20, flag21, flag22)}),
            }, group => group
  ...
  private contributeWorkbenchPartToolbarMenu(): void {
    contributeMenu('menu:workbench.part.toolbar', menu => {
      ...
      menu
        ...
        .addGroup(group => group
          .addMenuItem({label: 'Speed Search', icon: 'scion.search', accelerator: {ctrl: true, key: 'F'}, onSelect: () => console.log(`>>> Ctrl+F [location=menu:workbench.part.toolbar, contributor=${part.id}]`)}),
        )
        .addGroup(group => group
  ...
  private contributeWorkbenchDialogToolbarMenu(): void {
    contributeMenu('menu:workbench.dialog.toolbar', menu => {
      ...
      return menu
        .addMenuItem({label: 'Settings...', onSelect})
        .addGroup(group => group
