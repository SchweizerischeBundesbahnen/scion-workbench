...
export default class RegisterRoutePageComponent {
  ...
  protected readonly form = form(signal(this._initialFormValue), path => {
    ...
    hidden(path.canMatch.qualifier, {when: ({valueOf}) => !isCanMatchWorkbenchCapabilityFn(valueOf(path.canMatch.fn))});
    hidden(path.canMatch.hint, {when: ({valueOf}) => !isCanMatchWorkbenchElementFn(valueOf(path.canMatch.fn))});
  });
