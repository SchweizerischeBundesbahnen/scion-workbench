...
import {Component, computed, effect, inject, Injector, inputBinding, runInInjectionContext, signal, Signal, untracked} from '@angular/core';
...
export default class ViewPageComponent {
  ...
  private contributeWorkbenchPartToolbar(): void {
    ...
    contributeMenu('menu:workbench.part.toolbar', menu => menu
      ...
      .addGroup(group => group
        .addMenuItem({label: 'Speed Search', accelerator: {ctrl: true, key: 'F'}, icon: 'scion.search', onSelect: () => console.log(`>>> Speed Search [location=menu:workbench.part.toolbar, contributor=${this.view.id}]`)}),
      ),
    );
  }
  ...
  private contributeWorkbenchViewContextMenu(): void {
    contributeMenu('menu:workbench.view.contextmenu', menu => menu
      .addMenuItem({label: 'Speed Search', accelerator: {ctrl: true, key: 'F'}, icon: 'scion.search', onSelect: () => console.log(`>>> Speed Search [location=menu:workbench.view.contextmenu, contributor=${this.view.id}]`)}),
    );
  }
