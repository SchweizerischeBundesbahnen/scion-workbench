...
export default class ViewPageComponent {
  ...
  private contributeMenubar(): void {
    contributeMenu('menubar:view', menubar => menubar
      .addMenu({label: 'Menu 1'}, menu => menu
        .addMenuItem({label: 'Menu 1 - A', onSelect})
        .addMenuItem({label: 'Menu 1 - B', onSelect})
        .addMenuItem({label: 'Menu 1 - C', onSelect}),
      )
      .addMenu({label: 'Menu 2', menu: {name: 'menu:2'}}, menu => menu
        .addMenuItem({label: 'Menu 2 - A', onSelect})
        .addMenuItem({label: 'Menu 2 - B', onSelect})
        .addMenuItem({label: 'Menu 2 - C', onSelect}),
      )
      .addMenu({label: 'Menu 3'}, menu => menu
        .addMenuItem({label: 'Menu 3 - A', onSelect})
        .addMenuItem({label: 'Menu 3 - B', onSelect})
        .addMenuItem({label: 'Menu 3 - C', onSelect}),
      ),
    );

    contributeMenu({location: 'menubar:view', position: 'start'}, menu => menu
      .addMenu({label: 'File', menu: {filter: {placeholder: 'hello', notFoundMessage: 'nüd found'}}}, menu => menu
        ...
        .addMenu({label: 'Share', icon: 'person_add', menu: {name: 'menu:share'}}, menu => menu
          ...
          .addMenu({label: 'Text', icon: 'format_bold'}, menu => menu
            ...
            .addMenu({label: 'Size', icon: 'format_bold'}, menu => menu
              .addMenuItem({label: 'Increase font size', onSelect})
              .addMenuItem({label: 'Decrease font size', onSelect}),
            ),
          ),
        ),
      ),
    ...
    contributeMenu({location: 'menubar:view', position: 'end'}, menubar => menubar
      .addMenu({label: 'Menu 4', menu: {name: 'menu:view.menubar:additions'}}, menu => menu),
    );
  }
  ...
  public contributeClientMenubar(): void {
    ...
    this._workbenchMenuService.contributeMenu('menu:view.menubar:additions', menu => menu
      ...
      .addMenu({label: 'Additions', menu: {name: 'menu:additions'}}, menu => menu)
      ...
      .addGroup(group => group
        .addMenuItem({label: 'Speed Search', icon: 'scion.search', onSelect: () => onSelect()}),
      ),
    );
  }
    ...
    contributeMenu('toolbar:view', toolbar => toolbar
      ...
      .addGroup(group => group
        ...
        .addToolbarSplitButton({icon: 'format_underlined', checked: underlined, tooltip: 'Underline', accelerator: {ctrl: true, key: 'u'}, onSelect: () => underlined.update(underlined => !underlined)}, menu => menu
          .addGroup(group => group
            .addMenuItem({icon: 'format_underlined', label: 'Solid', onSelect: () => setUnderlined('solid')})
            .addMenuItem({icon: 'format_underlined', label: 'Double', onSelect: () => setUnderlined('double')})
            .addMenuItem({icon: 'format_underlined', label: 'Dashed', onSelect: () => setUnderlined('dashed')})
            .addMenuItem({icon: 'format_underlined', label: 'Dotted', onSelect: () => setUnderlined('dotted')}),
          )
      ...
      .addGroup(group => group
        .addToolbarButton({icon: 'undo', onSelect: () => onSelect()})
        .addToolbarButton({icon: 'redo', onSelect: () => onSelect()})
        .addToolbarButton({icon: 'content_cut', onSelect: () => onSelect()})
        .addToolbarButton({icon: 'content_copy', onSelect: () => onSelect()})
        .addToolbarButton({icon: 'content_paste', onSelect: () => onSelect()}),
      ),
    );
  ...
  public contributeClientToolbar(): void {
    ...
    this._workbenchMenuService.contributeMenu('toolbar:view', toolbar => toolbar
      .addGroup(group => group
        .addToolbarButton({icon: 'format_bold', checked: bold, onSelect: () => bold.next(!bold.value)})
        .addToolbarButton({icon: 'format_italic', checked: italic, onSelect: () => italic.next(!italic.value)})
        .addToolbarButton({icon: 'format_underlined', checked: underlined, onSelect: () => underlined.next(!underlined.value)}),
      )
      .addToolbarMenu({icon: 'palette', menu: {filter: true}}, menu => menu
        .addGroup(group => group
          .addMenuItem({label: 'Bold', checked: bold, onSelect: () => bold.next(!bold.value)})
          .addMenuItem({label: 'Italic', checked: italic, onSelect: () => italic.next(!italic.value)})
          .addMenuItem({label: 'Underline', checked: underlined, onSelect: () => underlined.next(!underlined.value)})
          .addMenuItem({label: 'Strikethrough', checked: strikethrough, onSelect: () => strikethrough.next(!strikethrough.value)}),
        )
  ...
  protected toggleMenu1(): void {
    if (this.contributionRef1) {
      ...
      this.contributionRef1 = contributeMenu('toolbar:router-page', menu => {
        ...
        menu.addToolbarButton({icon: 'favorite', onSelect});
  ...
  protected toggleMenu2(): void {
    if (this.contributionRef2) {
      ...
      this.contributionRef2 = contributeMenu('toolbar:router-page', menu => {
        ...
        menu.addToolbarButton({label: 'train', onSelect});
      }, {requiredContext: new Map().set(WorkbenchMenuContexts.ViewId, undefined), injector: this._injector});
    }
  }
  ...
  public contributeClientPartToolbar(): void {
    this._workbenchMenuService.contributeMenu('toolbar:workbench.part.toolbar', menu => menu
      .addToolbarMenu({icon: 'computer', label: 'Client', menu: {filter: {placeholder: 'hello', notFoundMessage: 'nüd found'}}}, menu => menu
        .addMenuItem({label: 'New', icon: 'article', onSelect: () => onSelect()})
        .addMenuItem({label: 'Open', icon: 'folder', onSelect: () => onSelect()})
        .addMenuItem({label: 'Make a Copy', icon: 'file_copy', onSelect: () => onSelect()})
        .addMenu({label: 'Share', icon: 'person_add', menu: {name: 'menu:share'}}, menu => menu
  ...
  private contributePartToolbar(): void {
    ...
    contributeMenu('toolbar:workbench.part.toolbar', toolbar => toolbar
      .addToolbarMenu({icon: 'star', menu: {filter: {placeholder: 'hello', notFoundMessage: 'nüd found'}}}, menu => menu
  ...
  private contributeContextMenu(): void {
    ...
    contributeMenu('menu:contextmenu', (menu, context) => menu
      ...
      .addGroup(group => group
        ...
        .addGroup(group => group
          .addMenuItem({label: 'Speed Search', icon: 'scion.search', onSelect: () => console.log('>>> speed search (Ctrl + F)', context)}),
        )
        .addGroup(group => group
...
function addHeadingGroupActions(toolbar: SciToolbarFactory): void {
  toolbar
    .addToolbarButton({icon: 'favorite', onSelect})
    .addToolbarMenu({icon: 'scion.more_vertical', visualMenuIndicator: false}, menu => menu
      ...
      .addMenuItem({label: 'Don\'t Show Again', onSelect}),
    );
}
