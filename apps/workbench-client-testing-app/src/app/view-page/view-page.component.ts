...
export default class ViewPageComponent {
  ...
  constructor() {
    ...
    console.log(`>>> provide menu with label=${label} from app=${this._appSymbolicName}`);
    contributeMenu('toolbar:microfrontend-toolbar', menu => menu
      .addMenu({label: 'File', name: 'menu:blubber'}, menu => menu
        .addMenuItem({label: label}, () => this.onAction())),
    );

    // contributeMenu('menu:blubber', menu => menu.addMenuItem({label: label}, () => this.onAction()), {injector: this._injector});
    //
    // // setTimeout(() => {
