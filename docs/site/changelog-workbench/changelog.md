...
* **workbench/router:** adding support for closing views that match a pattern introduced a breaking change in the Workbench Router API.
  ...
  Note regarding microfrontend workbench clients: The communication protocol between host and client is backward compatible, so you can upgrade the host and clients independently.

  To migrate applications using @scion/workbench:
  ...
  #### WorkbenchRouter of @scion/workbench
  ...
  **Close view by providing a viewId (NEW)**
  ...
  #### WorkbenchRouter of @scion/workbench-client
