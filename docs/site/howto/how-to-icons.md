...
The SCION Workbench uses built-in icons in various places. Additionally, the application can use its own icons in the layout (e.g., icons for Workbench Parts).
...
The SCION Workbench installs a Material icon provider if no icon provider is configured, allowing the application to reference Material icon ligatures in the layout (e.g., icons for Workbench Parts).
...
The SCION Workbench uses built-in icons from an icon font. Download the icon font from [GitHub][icon-font], unzip the font and place the files in the `/public/fonts` folder.
