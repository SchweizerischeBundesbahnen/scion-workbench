...
function assertSubMenu(item: ExpectedSciMenu, actual: DebugElement): void {
  ...
  if (item.menu?.filter !== undefined) {
    const filterHtmlElement = actual.parent!.query(By.css('div.e2e-filter'))?.nativeElement;
    if (item.menu.filter === true) {
      if (!filterHtmlElement) {
        throw Error(`[DOMAssertError] Expected menu filter to be present in the DOM, but is not.`);
      }
    }
    else if (item.menu.filter === false) {
      if (filterHtmlElement) {
        throw Error(`[DOMAssertError] Expected menu filter not to be present in the DOM, but is.`);
      }
    }

    if (typeof item.menu.filter === 'object') {
      if (item.menu.filter.placeholder !== undefined) {
        const inputFilterHtmlElement = actual.parent!.query(By.css('input.e2e-menu-filter-input'))!.nativeElement as HTMLInputElement;
        if (inputFilterHtmlElement.placeholder !== item.menu.filter.placeholder) {
          throw Error(`[DOMAssertError] Expected menu filter placeholder to be '${item.menu.filter.placeholder}' but was ${inputFilterHtmlElement.placeholder}`);
        }
      }
    }
...
export interface ExpectedSciMenu {
  ...
  menu?: {
    ...
    maxWidth?: string;
    maxHeight?: string;
    filter?: boolean | {placeholder?: Translatable; notFoundText?: Translatable};
  };
  cssClass?: string[];
