...
describe('Toolbar', () => {
  ...
  it('should contribute to toolbar', async () => {
    ...
    await expectAsync(fixture).toEqualToolbar([
      {
        type: 'menu-item',
        iconLigature: 'icon-1',
      },
  ...
  it('should call toolbar factory function once (multiple toolbars)', async () => {
    ...
    // Expect toolbar.
    expect(toolbarFactoryFunction1CallCount).toBe(1);
    await expectAsync(fixture1).toEqualToolbar([
      {
        type: 'menu-item',
        iconLigature: 'icon-1',
      },
    ]);
    await expectAsync(fixture2).toEqualToolbar([
      {
        type: 'menu-item',
        iconLigature: 'icon-1',
      },
  ...
  describe('Menu Properties', () => {
    ...
    xit('should provide menu filter', async () => {
      TestBed.configureTestingModule({
        providers: [
          {provide: ComponentFixtureAutoDetect, useValue: true},
        ],
      ...
      // Contribute to toolbar.
      contributeMenu('toolbar:testee', toolbar => toolbar
          .addMenu({menu: {}, icon: 'icon-1', cssClass: 'testee-1'}, menu => menu.addMenuItem({label: 'label', onSelect: noop}))
          .addMenu({menu: {filter: false}, icon: 'icon-2', cssClass: 'testee-2'}, menu => menu.addMenuItem({label: 'label', onSelect: noop}))
          .addMenu({menu: {filter: true}, icon: 'icon-3', cssClass: 'testee-3'}, menu => menu.addMenuItem({label: 'label', onSelect: noop}))
        // .addMenu({menu: {filter: {placeholder: 'testee-placeholder'}}, label: 'label-1'}, menu => menu.addMenuItem({label: 'label', onSelect: noop}))
        // .addMenu({menu: {filter: {notFoundText: 'testee-notFoundText'}}, label: 'label-1'}, menu => menu.addMenuItem({label: 'label', onSelect: noop}))
        ,
        {injector: TestBed.inject(Injector)});
      await fixture.whenStable();
