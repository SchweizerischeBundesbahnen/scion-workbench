...
export class ViewDragService implements OnDestroy {
  ...
  /**
   * Determines if a view can be dropped on the specified target.
   */
  public canDrop(target: ɵWorkbenchPart | MPartGrid | Signal<ɵWorkbenchPart | MPartGrid>): Signal<boolean> {
    return computed(() => {
      ...
      return untracked(() => {
        ...
        // Allow drop only if the view is dragged within the same activity.
      ...
      /**
       * Gets the activity that contains the specified element, or `null` if the element is not contained within an activity.
       */
  ...
  /**
   * Signals when dragging a view tab.
   */
  private installDragSignaler(): void {
    effect(() => {
