...
import {BehaviorSubject, concatWith, delay, firstValueFrom, map, merge, of, Subject, switchMap} from 'rxjs';
...
export class ɵWorkbenchDialog<R = unknown> implements WorkbenchDialog<R>, Blockable, Blocking {
  ...
  private stickToHostElement(): void {
    if (this._options.modality === 'application' && this._workbenchConfig.dialog?.modalityScope === 'viewport') {
      ...
      effect(() => {
        // Maintain position and size when detached to prevent flickering when re-attached to the host and to support for virtual scrolling in dialog content.
        if (this.context.view && !this.context.view.portal.attached()) {
          return;
        }

        // IMPORTANT: Track host bounds only if attached to prevent flickering.
        if (!hostBounds()) {
          return;
        }
