...
import {Component, computed, effect, ElementRef, inject, input, signal, Signal, TemplateRef, viewChild, ViewContainerRef} from '@angular/core';
...
export class ContentAsOverlayComponent {
  ...
  private createOverlay(): void {
    effect(onCleanup => {
      ...
      const overlayViewRef = location.createEmbeddedView(this._template(), null);
      ...
      onCleanup(() => overlayViewRef.destroy());
  ...
  private alignOverlayToHostBounds(): void {
    ...
    effect(() => {
      ...
      // and to support for virtual scrolling in projected content.
...
export interface ContentAsOverlayConfig {
   ...
   */
  location: Signal<ViewContainerRef | undefined | null>;
  /**
