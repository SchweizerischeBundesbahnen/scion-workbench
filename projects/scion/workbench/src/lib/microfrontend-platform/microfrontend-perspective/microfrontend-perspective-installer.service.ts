...
export class MicrofrontendPerspectiveInstaller {
   ...
   * - If no part capability is found, creates an empty part and logs an error.
  ...
  private async addInitialPart(partRef: Omit<WorkbenchPartRef, 'position'>, perspectiveCapability: WorkbenchPerspectiveCapability, layoutFactory: WorkbenchLayoutFactory): Promise<WorkbenchLayout> {
    ...
    if (!partCapability) {
      return layoutFactory.addPart(partRef.id, {cssClass: partRef.cssClass});
    }
  ...
  private async addPart(partRef: WorkbenchPartRef, perspectiveCapability: WorkbenchPerspectiveCapability, layout: WorkbenchLayout): Promise<WorkbenchLayout> {
    ...
    if (typeof partRef.position === 'string') { // docked part
      ...
      this._logger.error(`[PerspectiveDefinitionError] Perspective '${qualifier(perspectiveCapability)}' of app '${app(perspectiveCapability)}' aligns part '${partRef.id}' relative to missing part '${partRef.position.relativeTo}'. The part id may be incorrect or the part not available. Ignoring part.`, LoggerNames.MICROFRONTEND);
      return layout;
    }
