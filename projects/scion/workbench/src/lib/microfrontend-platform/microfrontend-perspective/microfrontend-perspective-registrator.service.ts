...
export class MicrofrontendPerspectiveRegistrator {
  ...
  private installPerspectiveCapabilityListener(): void {
    ...
    this._manifestService.lookupCapabilities$<WorkbenchPerspectiveCapability>({type: WorkbenchCapabilities.Perspective})
      ...
      .subscribe(perspectiveCapabilities => {
        differ.diff(perspectiveCapabilities)?.forEachAddedItem(({item: perspectiveCapability}) => {
  ...
  private createLayout(perspectiveCapability: WorkbenchPerspectiveCapability): WorkbenchLayoutFn {
    return async (factory: WorkbenchLayoutFactory): Promise<WorkbenchLayout> => {
      ...
      for (const part of [initialPart, ...parts]) {
        ...
        const activeView = part.views.find(view => view.active) ?? part.views[0];

        for (const [viewIndex, view] of part.views.entries()) {
          const viewCapabilities = await this.lookupViewCapabilities(view.qualifier, {
            perspectiveContributor: perspectiveCapability.metadata!.appSymbolicName,
          ...
          viewCapabilities
            ...
            .forEach(viewCapabilityId => {
              ...
              layout = layout
                .addView(alternativeViewId, {partId: part.id, activateView: view === activeView})
                .navigateView(alternativeViewId, commands, {cssClass: view.cssClass});
            });

          // Navigate to the "~" route if not finding a view capability, e.g., because the perspective contributor references a private view capability
  ...
  /**
   * Searches for views that match the specified qualifier and are accessible to the perspective contributor,
   * i.e., its own view capabilities or public view capabilities of other applications the perspective contributor
   * has an intention for.
   */
  private async lookupViewCapabilities(qualifier: Qualifier, perspective: {perspectiveContributor: string; perspectiveQualifier: Qualifier}): Promise<WorkbenchViewCapability[]> {
    const viewCapabilities$ = this._manifestService.lookupCapabilities$<WorkbenchViewCapability>({type: WorkbenchCapabilities.View, qualifier})
      .pipe(filterArray(async viewCapability => {
        const allowed = await firstValueFrom(this._manifestService.isApplicationQualified$(perspective.perspectiveContributor, {capabilityId: viewCapability.metadata!.id}));
        if (!allowed) {
          this._logger.warn(`[NotQualifiedError] Application '${(perspective.perspectiveContributor)}' is not allowed to reference the view '${Objects.toMatrixNotation(viewCapability.qualifier)}' in the perspective '${Objects.toMatrixNotation(perspective.perspectiveQualifier)}'. Ensure you have specified an intention and that the view is public.`, LoggerNames.MICROFRONTEND);
        }
        return allowed;
      }));
