...
export class MicrofrontendMessageBoxCapabilityValidator implements CapabilityInterceptor {
  ...
  public async intercept(capability: Capability): Promise<Capability> {
    ...
    if (path === undefined || path === null) {
      throw Error(`[NullPathError] MessageBox capability requires a path to the microfrontend in its properties [capability=${Objects.toMatrixNotation(messageBoxCapability.qualifier)}]`);
    }
