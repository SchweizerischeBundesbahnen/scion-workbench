...
export class MicrofrontendMessageBoxCapabilityValidator implements CapabilityInterceptor {
  ...
  public async intercept(capability: Capability): Promise<Capability> {
    ...
    if (!messageBoxCapability.properties) {
      throw Error(`[MessageBoxDefinitionError] MessageBox capability requires a 'properties' section [app=${app(messageBoxCapability)}, messagebox=${qualifier(messageBoxCapability)}]`);
    }
