...
export class MicrofrontendHostPart implements ActivatedMicrofrontend {
  ...
  constructor(part: ɵWorkbenchPart) {
    ...
    this.capability = this.computePartCapability(navigationData);
  ...
  private computePartCapability(navigationData: Signal<MicrofrontendPartNavigationData>): Signal<WorkbenchPartCapability> {
    const capability = inject(ManifestObjectCache).observeCapability<WorkbenchPartCapability>(computed(() => navigationData().capabilityId));
  ...
  /**
   * Instructs Angular to evaluate `CanMatch` route guards to display the "Not Found" page if unregistering the part capability while the part is open.
   */
