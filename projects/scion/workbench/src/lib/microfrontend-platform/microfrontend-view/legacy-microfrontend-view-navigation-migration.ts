...
class MicrofrontendViewNavigationMigrationComponent {
  ...
  private async migrateView(): Promise<void> {
    ...
    await router.navigate(layout => {
      return layout.navigateView(view.id, [], {
 ...
 */
function parseLegacyMicrofrontendViewUrl(segments: UrlSegment[]): {legacyCapabilityId: string; params: Params} | null {
