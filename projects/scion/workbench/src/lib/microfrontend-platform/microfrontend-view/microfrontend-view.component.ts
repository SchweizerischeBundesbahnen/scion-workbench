...
import {ChangeDetectionStrategy, ChangeDetectorRef, Component, computed, CUSTOM_ELEMENTS_SCHEMA, DestroyRef, ElementRef, inject, Injector, linkedSignal, Provider, signal, Signal, untracked, viewChild} from '@angular/core';
...
export class MicrofrontendViewComponent {
  ...
  private propagateKeyboardEvents(): void {
    toObservable(this._routerOutletElement)
      .pipe(
        switchMap(routerOutletElement => fromEvent<KeyboardEvent>(routerOutletElement.nativeElement, 'keydown')),
        takeUntilDestroyed(),
      )
      .subscribe(event => {
