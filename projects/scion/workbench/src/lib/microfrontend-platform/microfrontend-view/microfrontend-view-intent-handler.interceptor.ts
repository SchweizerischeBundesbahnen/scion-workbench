...
export class MicrofrontendViewIntentHandler implements IntentInterceptor {
  ...
  private findViews(intentMessage: IntentMessage, extras: WorkbenchNavigationExtras, options: {matchWildcardParams: boolean}): string[] | null {
    ...
    const viewIds = layout
      .views({
        peripheral: extras.partId ? undefined : false,
        partId: extras.partId,
      })
      .filter(mView => {
        const viewUrlSegments = layout.urlSegments({outlet: mView.id});
        const microfrontendURL = MicrofrontendViewRoutes.parseMicrofrontendURL(viewUrlSegments);
        if (!microfrontendURL) {
          return false;
        }
        ...
        return requiredParams.every(({name}) => {
          ...
          return (options.matchWildcardParams && intentParamValue === '*') || intentParamValue === microfrontendURL.params[name];
        });
