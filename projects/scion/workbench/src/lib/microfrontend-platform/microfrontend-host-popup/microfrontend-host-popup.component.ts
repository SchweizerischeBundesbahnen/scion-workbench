...
export class MicrofrontendHostPopupComponent {
  ...
  constructor() {
    const popupContext = this.popup.input as ɵPopupContext;
  ...
  private computeOutletInjector(): Injector {
    const injector = inject(Injector);
    const popup = inject(ɵPopup);

    return Injector.create({
      parent: injector,
      providers: [provideWorkbenchPopupHandle(popup.input as ɵPopupContext)],
    });
  }
}
