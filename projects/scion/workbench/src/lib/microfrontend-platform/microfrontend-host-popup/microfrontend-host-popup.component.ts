...
export class MicrofrontendHostPopupComponent implements OnDestroy {
  ...
  constructor(popup: ɵPopup<ɵPopupContext>,
    ...
    const path = Defined.orElseThrow(capability.properties.path, () => Error(`[PopupProviderError] Missing required path for popup capability: ${Objects.toMatrixNotation(capability.qualifier)}`));
