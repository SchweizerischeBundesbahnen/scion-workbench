...
export class MicrofrontendHostView implements ActivatedMicrofrontend {
  ...
  constructor(view: ɵWorkbenchView) {
    ...
    this.capability = this.computeViewCapability(navigationData);
  ...
  private computeViewCapability(navigationData: Signal<MicrofrontendViewNavigationData>): Signal<WorkbenchViewCapability> {
    const capability = inject(ManifestObjectCache).observeCapability<WorkbenchViewCapability>(computed(() => navigationData().capabilityId));
  ...
  /**
   * Instructs Angular to evaluate `CanMatch` route guards to display the "Not Found" page if unregistering the view capability while the view is open.
   */
