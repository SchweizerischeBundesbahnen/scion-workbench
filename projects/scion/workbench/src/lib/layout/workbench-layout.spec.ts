...
describe('WorkbenchLayout', () => {
  ...
  it('should remove non-structural part when removing its last view', () => {
    ...
    const workbenchLayout = TestBed.inject(ɵWorkbenchLayoutFactory)
      .addPart(MAIN_AREA)
      .addPart('part.left', {relativeTo: 'part.main', align: 'left'}, {structural: false})
  ...
  it('should not remove structural part when removing its last view', () => {
    ...
    const workbenchLayout = TestBed.inject(ɵWorkbenchLayoutFactory)
      .addPart(MAIN_AREA)
      .addPart('part.left', {relativeTo: 'part.main', align: 'left'}) // structural by default if not set
      .addView('view.1', {partId: 'part.left'})
      .addView('view.2', {partId: 'part.left'})
      .removeView('view.1')
      .removeView('view.2');
  ...
  it('should add view navigation data to the layout', () => {
    ...
    expect(workbenchLayout.part({partId: 'part.main'}).views).toEqual(jasmine.arrayWithExactContents([
  ...
  it('should add part navigation data to the layout', () => {
    const workbenchLayout = TestBed.inject(ɵWorkbenchLayoutFactory)
      .addPart('part.main')
      .navigatePart('part.main', ['path/to/part'], {data: {some: 'data'}});

    expect(workbenchLayout.part({partId: 'part.main'}).navigation).toEqual({id: anything(), data: {some: 'data'}} satisfies MPart['navigation']);
  });
  ...
  it('should add part navigation state to the layout', () => {
    const workbenchLayout = TestBed.inject(ɵWorkbenchLayoutFactory)
      .addPart('part.main')
      .navigatePart('part.main', ['path/to/part'], {state: {some: 'state'}});

    expect(workbenchLayout.part({partId: 'part.main'}).navigation).toEqual({id: anything()} satisfies MPart['navigation']);
    expect(workbenchLayout.navigationState({outlet: 'part.main'})).toEqual({some: 'state'});
  });
  ...
  it('should error when navigating non-existent part', () => {
    const workbenchLayout = TestBed.inject(ɵWorkbenchLayoutFactory).addPart('part.main');
  ...
  it('should compute next view id', async () => {
    ...
    workbenchLayout = workbenchLayout.addView('view.2', {partId: 'part.main'});
    expect(workbenchLayout.computeNextViewId()).toEqual('view.3');

    workbenchLayout = workbenchLayout.addView('view.3', {partId: 'part.main'});
    expect(workbenchLayout.computeNextViewId()).toEqual('view.4');

    workbenchLayout = workbenchLayout.addView('view.4', {partId: 'part.main'});
    expect(workbenchLayout.computeNextViewId()).toEqual('view.5');

    workbenchLayout = workbenchLayout.addView('view.5', {partId: 'part.main'});
    expect(workbenchLayout.computeNextViewId()).toEqual('view.6');

    workbenchLayout = workbenchLayout.addView('view.6', {partId: 'part.main'});
  ...
  it('should find parts by criteria', () => {
    ...
    const workbenchLayout = TestBed.inject(ɵWorkbenchLayoutFactory)
      .addPart(MAIN_AREA)
      .addPart('part.innerLeft', {relativeTo: 'part.main', align: 'left'})
      .addPart('part.innerRight', {relativeTo: 'part.main', align: 'right'})
  ...
  it('should throw an error if not finding the part', () => {
    ...
    expect(() => workbenchLayout.part({partId: 'part.main', viewId: 'view.2'})).toThrowError(/NullPartError/);
    expect(() => workbenchLayout.part({grid: 'workbench', partId: 'part.main', viewId: 'view.1'})).toThrowError(/NullPartError/);
    expect(() => workbenchLayout.part({grid: 'workbench', viewId: 'view.1'})).toThrowError(/NullPartError/);
    expect(() => workbenchLayout.part({grid: 'workbench', partId: 'part.main'})).toThrowError(/NullPartError/);
  ...
  it('should return `null` if not finding the part', () => {
    ...
    expect(workbenchLayout.part({partId: 'part.main', viewId: 'view.2'}, {orElse: null})).toBeNull();
    expect(workbenchLayout.part({grid: 'workbench', partId: 'part.main', viewId: 'view.1'}, {orElse: null})).toBeNull();
    expect(workbenchLayout.part({grid: 'workbench', viewId: 'view.1'}, {orElse: null})).toBeNull();
    expect(workbenchLayout.part({grid: 'workbench', partId: 'part.main'}, {orElse: null})).toBeNull();
  });
  ...
  it('should return whether a part is contained in the main area', () => {
    ...
    const workbenchLayout = TestBed.inject(ɵWorkbenchLayoutFactory)
      .addPart(MAIN_AREA)
      .addPart('part.inner', {relativeTo: 'part.main', align: 'left'})
      .addPart('part.outer', {relativeTo: MAIN_AREA, align: 'left'});

    expect(workbenchLayout.hasPart('part.main', {grid: 'mainArea'})).toBeTrue();
  ...
  it('should activate adjacent view', () => {
    ...
    let workbenchLayout = TestBed.inject(ɵWorkbenchLayoutFactory)
      .addPart(MAIN_AREA)
      .addPart('part.part', {relativeTo: 'part.main', align: 'left'})
      .addView('view.1', {partId: 'part.part'})
      .addView('view.2', {partId: 'part.part'})
      .addView('view.3', {partId: 'part.part'});

    expect(workbenchLayout.activePart({grid: 'mainArea'})!.id).toEqual('part.main');
