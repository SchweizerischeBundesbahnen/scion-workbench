...
export class WorkbenchLayoutMigrationV6 implements WorkbenchMigration {
  ...
  public migrate(json: string): string {
    ...
    const workbenchLayoutV6: MWorkbenchLayoutV6 = {
      userLayout: {
        grids: workbenchLayoutSerializer.serializeGrids(userLayoutV6.grids),
        activityLayout: workbenchLayoutSerializer.serializeActivityLayout(userLayoutV6.activityLayout),
        outlets: workbenchLayoutSerializer.serializeOutlets(userLayoutV6.outlets({mainGrid: true, activityGrids: true})),
      },
      referenceLayout: {
        grids: workbenchLayoutSerializer.serializeGrids(referenceLayoutV6.grids),
        activityLayout: workbenchLayoutSerializer.serializeActivityLayout(referenceLayoutV6.activityLayout),
        outlets: workbenchLayoutSerializer.serializeOutlets(referenceLayoutV6.outlets({mainGrid: true, activityGrids: true})),
      },
    };
  ...
  private migrateReferencePart(layout: Layout): ɵWorkbenchLayout {
    ...
    const migratedActivityGrids = WorkbenchLayouts.pickActivityGrids(layout.grids, grid => {
