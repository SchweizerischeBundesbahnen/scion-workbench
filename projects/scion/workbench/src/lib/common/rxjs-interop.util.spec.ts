...
import {rootEffect, toRootObservable} from './rxjs-interop.util';
...
describe('RxJS Interop', () => {
  ...
  describe('toRootObservable', () => {

    it('should emit when tracked signal change even if detached from change detection tree', async () => {
      const testSignal = signal(0);
      let observable!: Observable<number>;

      // Create root observable inside a component context.
      const {fixture, component} = createDynamicTestComponent(() => {
        observable = toRootObservable(testSignal);
      });

      const captor = new ObserveCaptor();
      observable.subscribe(captor);

      // Expect initial emission.
      await fixture.whenStable();
      expect(captor.getValues()).toEqual([0]);
      ...
      // Expect emission.
      await fixture.whenStable();
      expect(captor.getValues()).toEqual([0, 1]);
      ...
      // Expect emission.
      await fixture.whenStable();
      expect(captor.getValues()).toEqual([0, 1, 2]);

      // Change tracked signal.
      testSignal.set(3);
      await fixture.whenStable();
      expect(captor.getValues()).toEqual([0, 1, 2, 3]);
      ...
      // Expect emission.
      await fixture.whenStable();
      expect(captor.getValues()).toEqual([0, 1, 2, 3, 4]);
    });
    ...
    it('should unregister effect when injection context is destroyed', async () => {
      ...
      const observable = runInInjectionContext(injector, () => toRootObservable(testSignal));
      TestBed.tick();
    ...
    it('should unregister effect when passed injector is destroyed', async () => {
      ...
      // Create root observable.
      const observable = toRootObservable(testSignal, {injector});
      TestBed.tick();
