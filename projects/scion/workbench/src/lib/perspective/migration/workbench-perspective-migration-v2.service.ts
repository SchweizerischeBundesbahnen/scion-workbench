...
import {MPerspectiveLayoutsV2} from './model/workbench-perspective-migration-v2.model';
...
export class WorkbenchPerspectiveMigrationV2 implements WorkbenchMigration {
  ...
  public migrate(json: string): string {
    ...
    const perspectiveLayoutsV2: MPerspectiveLayoutsV2 = {
      userLayout: {
    ...
    return JSON.stringify(perspectiveLayoutsV2);
  }
  ...
  private migrateViewOutlets(viewOutlets: {[viewId: string]: Commands}): string {
    return JSON.stringify(Object.fromEntries(Object.entries(viewOutlets)
      .map(([viewId, commands]: [string, Commands]): [string, UrlSegmentDataV2[]] => {
...
function commandsToSegments(commands: Commands): UrlSegmentDataV2[] {
  const segments = new Array<UrlSegmentDataV2>();
