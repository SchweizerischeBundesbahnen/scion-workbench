...
describe('WorkbenchGridMerger', () => {
  ...
  it('should discard local changes when diff between base and remote desktop', () => {
    const base = TestBed.inject(ɵWorkbenchLayoutFactory)
      .addPart(MAIN_AREA)
      .addPart('left', {align: 'left'})
      .addView('view.1', {partId: 'left'})
      .navigateView('view.1', ['path/to/view'])
      .navigateDesktop(['path/to/desktop'], {hint: 'hint-a'});
    ...
    const mergedLayout = TestBed.inject(WorkbenchGridMerger).merge({
      ...
      base,
      remote: base.navigateDesktop(['new/path/to/desktop'], {hint: 'hint-b'}),
    });
    ...
    expect(mergedLayout).toEqualWorkbenchLayout({
      workbenchGrid: {
        root: new MTreeNode({
          ...
          child1: new MPart({
            id: 'left',
            views: [
              {id: 'view.1', navigation: {id: ANYTHING}},
            ],
          }),
          child2: new MPart({id: MAIN_AREA}),
        }),
      ...
      desktop: {
        navigation: {id: ANYTHING, hint: 'hint-b'},
      },
    });

    expect(mergedLayout.outlets()).toEqual({
      'view.1': segments(['path/to/view']),
      [DESKTOP_OUTLET]: segments(['new/path/to/desktop']),
    });
  });
});
