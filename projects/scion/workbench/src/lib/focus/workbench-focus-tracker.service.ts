...
export function trackFocus(target: HTMLElement, workbenchElement: WorkbenchElement | null): FocusTrackerRef {
  ...
  return {
    ...
    destroy: () => {
      focusMonitor.unsetActiveElement(workbenchElement);
...
export interface FocusTrackerRef {
  ...
  /**
   * Unsets the active workbench element if the tracked element was the focus owner and destroys the tracker ref.
   */
  destroy(): void;
}
