...
import {Component, computed, ElementRef, inject, Injectable, Injector, input, inputBinding, runInInjectionContext, signal, TemplateRef} from '@angular/core';
...
export class WorkbenchViewContextMenuService {
  ...
  public open(event: MouseEvent, context: {viewId: ViewId}): void {
    this._menuService.open('menu:workbench.view.contextmenu.internal', {
      anchor: event,
      width: 'var(--sci-workbench-view-contextmenu-width)',
      context: this.createViewMenuContext(context.viewId), // Pass menu context to be independent of the invocation context.
      cssClass: 'e2e-view-contextmenu',
      attributes: {'data-viewid': context.viewId},
    });
  }
  ...
  public registerBuiltInMenuItems(): void {
    ...
    const showBuiltInMenus = config !== false;
    ...
    contributeMenu({location: 'menu:workbench.view.contextmenu.internal'}, menu => {
      ...
      // Add 'close' group.
      showBuiltInMenus && menu.addGroup(group => {
      ...
      // Add 'move' group.
      showBuiltInMenus && menu.addGroup({name: 'menu:workbench.view.contextmenu.internal:move'}, group => {
  ...
  public registerLegacyMenuContribution(legacyViewMenuItemFn: WorkbenchViewMenuItemFn): Disposable {
    const contribution = contributeMenu('menu:workbench.view.contextmenu', group => {
      ...
      group.addMenuItem({
        label: coerceComponent(legacyViewMenuItem, view),
    ...
    function coerceComponent(legacyViewMenuItem: WorkbenchMenuItem, view: WorkbenchView): SciComponentDescriptor {
      if (legacyViewMenuItem.content instanceof TemplateRef) {
        return {
          component: MenuItemComponent,
          bindings: [
            inputBinding('template', signal(legacyViewMenuItem.content)),
            inputBinding('context', signal({$implicit: view, ...legacyViewMenuItem.inputs})),
          ],
  ...
  private registerCloseMenuItem(config: MenuItemConfig | LegacyMenuItemConfig | false, group: SciMenuFactory, view: WorkbenchView): void {
    if (config) {
      group.addMenuItem({
        label: '%scion.workbench.close_tab.action',
        accelerator: parseLegacyMenuAccelerator(config.accelerator) ?? {ctrl: true, key: 'K'},
  ...
  private registerCloseOtherTabsMenuItem(config: MenuItemConfig | LegacyMenuItemConfig | false, group: SciMenuFactory, view: WorkbenchView): void {
    if (config) {
      group.addMenuItem({
        label: '%scion.workbench.close_other_tabs.action',
        accelerator: parseLegacyMenuAccelerator(config.accelerator) ?? {ctrl: true, shift: true, key: 'K'},
  ...
  private registerCloseAllTabsMenuItem(config: MenuItemConfig | LegacyMenuItemConfig | false, group: SciMenuFactory, view: WorkbenchView): void {
    if (config) {
      group.addMenuItem({
        label: '%scion.workbench.close_all_tabs.action',
        accelerator: parseLegacyMenuAccelerator(config.accelerator) ?? {ctrl: true, shift: true, alt: true, key: 'K'},
  ...
  private registerCloseRightTabsMenuItem(config: MenuItemConfig | LegacyMenuItemConfig | false, group: SciMenuFactory, view: WorkbenchView): void {
    if (config) {
      group.addMenuItem({
        label: '%scion.workbench.close_tabs_to_the_right.action',
        accelerator: parseLegacyMenuAccelerator(config.accelerator),
  ...
  private registerCloseLeftTabsMenuItem(config: MenuItemConfig | LegacyMenuItemConfig | false, group: SciMenuFactory, view: WorkbenchView): void {
    if (config) {
      group.addMenuItem({
  ...
  private registerMoveRightMenuItem(config: MenuItemConfig | LegacyMenuItemConfig | false, group: SciMenuFactory, view: WorkbenchView): void {
    if (config) {
      group.addMenuItem({
        label: '%scion.workbench.move_tab_to_the_right.action',
        accelerator: {ctrl: true, alt: true, key: 'End'},
  ...
  private registerMoveLeftMenuItem(config: MenuItemConfig | LegacyMenuItemConfig | false, group: SciMenuFactory, view: WorkbenchView): void {
    if (config) {
      group.addMenuItem({
  ...
  private registerMoveUpMenuItem(config: MenuItemConfig | LegacyMenuItemConfig | false, group: SciMenuFactory, view: WorkbenchView): void {
    if (config) {
      group.addMenuItem({
  ...
  private registerMoveDownMenuItem(config: MenuItemConfig | LegacyMenuItemConfig | false, group: SciMenuFactory, view: WorkbenchView): void {
    if (config) {
      group.addMenuItem({
  ...
  private registerMoveToNewWindowMenuItem(config: MenuItemConfig | LegacyMenuItemConfig | false, group: SciMenuFactory, view: WorkbenchView): void {
...
/**
 * TODO [Angular 23] Remove when removing support for legacy menu accelerator.
 */
function parseLegacyMenuAccelerator(accelerator: SciKeyboardAccelerator | string[] | undefined): SciKeyboardAccelerator | undefined {
  ...
  const segments = accelerator.map(segment => segment);
  const modifierKeys = new Set(segments.slice(0, -1));
  return {
    key: segments.at(-1)!,
    ctrl: modifierKeys.has('ctrl') || modifierKeys.has('control') || modifierKeys.has('meta'),
    shift: modifierKeys.has('shift'),
    alt: modifierKeys.has('alt'),
  };
}
