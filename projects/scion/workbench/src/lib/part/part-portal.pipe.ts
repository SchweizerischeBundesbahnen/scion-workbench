...
/**
 * Resolves the portal for the given part, or throws an error if the part is not registered.
 */
...
export class PartPortalPipe implements PipeTransform {
  ...
  // public transform(partId: PartId | null): Signal<WbComponentPortal<PartComponent | MainAreaPartComponent> | null> {
  //   if (!partId) {
  //     return signal(null);
  //   }
  //   return computed(() => this._partRegistry.objects().find(part => part.id === partId)?.portal ?? null);
  // }
