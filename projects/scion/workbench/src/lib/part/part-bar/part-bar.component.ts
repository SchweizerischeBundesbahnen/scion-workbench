...
export class PartBarComponent {
  ...
  private readonly _tabbar = viewChild.required(ViewTabBarComponent, {read: ElementRef<HTMLElement>});
  ...
  protected readonly maxTabBarWidth: Signal<number>;

  constructor() {
    this.maxTabBarWidth = this.calculateMaxTabBarWidth();
  }
  ...
  /**
   * Calculates the maximum available width for the tab bar.
   */
  private calculateMaxTabBarWidth(): Signal<number> {
    const fillerDimension = dimension(this._fillerElement);
    const tabbarDimension = dimension(this._tabbar);
    return computed(() => tabbarDimension().clientWidth + fillerDimension().offsetWidth);
  }
}
