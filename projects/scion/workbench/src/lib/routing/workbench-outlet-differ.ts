...
export class WorkbenchOutletDiffer {
  ...
  public diff(workbenchLayout: ɵWorkbenchLayout | null, urlTree: UrlTree): WorkbenchOutletDiff {
    // Combine view outlets from the URL and the layout. We require both sources because the layout is not
    ...
    // Combine part outlets from the URL and the layout. We require both sources because the layout is not
    // available during initial navigation, and the URL does not contain outlets for empty-path navigations.
    const partOutlets = [
