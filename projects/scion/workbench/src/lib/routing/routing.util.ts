...
export const Routing = {
  ...
  pathToCommands: (path: string): Commands => {
    ...
    const segments = urlTree.root.children[PRIMARY_OUTLET]?.segments as UrlSegment[] | undefined; // eslint-disable-line @typescript-eslint/no-unnecessary-condition
    if (!segments) {
  ...
  lookupRouteData: <T>(activatedRoute: ActivatedRouteSnapshot, dataKey: string): T | undefined => {
    return activatedRoute.pathFromRoot.reduceRight((resolvedData, route) => resolvedData ?? route.data[dataKey] as T, undefined as T);
  },
