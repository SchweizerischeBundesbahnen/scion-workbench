 ...
 *
 * Flags:
 * - matchWildcardPathSegment: Indicates if wildcard path matching is enabled. If enabled, the asterisk `*` path segment matches any segment.
 * - matchMatrixParams: Controls whether to match matrix parameters.
 */
export class UrlSegmentMatcher {

  constructor(private _pattern: UrlSegment[], private _flags: {matchWildcardPathSegment: boolean; matchMatrixParams: boolean}) {
  }
  ...
  public matches(segments: UrlSegment[]): boolean {
    ...
    return segments.every((segment, index) => {
      ...
      const checkPathSegment = !this._flags.matchWildcardPathSegment || this._pattern[index].path !== '*';

      if (checkPathSegment && segment.path !== this._pattern[index].path) {
        return false;
      }
