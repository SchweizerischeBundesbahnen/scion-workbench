...
describe('UrlSegmentMatcher', () => {
  ...
  describe('Flags: {matchMatrixParams: false}', () => {
    it('should match segments (pattern without matrix params)', () => {
      const matcher = new UrlSegmentMatcher(segments(['a', 'b', 'c']), {matchMatrixParams: false, matchWildcardPathSegment: false});
    ...
    it('should match segments (pattern with matrix params)', () => {
      const matcher = new UrlSegmentMatcher(segments(['a', 'b', 'c', {param1: 'value1', param2: 'value2'}]), {matchMatrixParams: false, matchWildcardPathSegment: false});
      expect(matcher.matches(segments(['a', 'b', 'c']))).toBeTrue();
      expect(matcher.matches(segments(['a', 'b', 'c', {param1: 'value1', param2: 'value2'}]))).toBeTrue();
      expect(matcher.matches(segments(['a', 'c', 'b']))).toBeFalse();
      expect(matcher.matches(segments(['a', 'b']))).toBeFalse();
  ...
  describe('Flags: {matchMatrixParams: true}', () => {
    it('should match segments (pattern without matrix params)', () => {
      const matcher = new UrlSegmentMatcher(segments(['a', 'b', 'c']), {matchMatrixParams: true, matchWildcardPathSegment: false});
      expect(matcher.matches(segments(['a', 'b', 'c']))).toBeTrue();
      expect(matcher.matches(segments(['a', 'b', 'c', {param1: 'value1', param2: 'value2'}]))).toBeFalse();
    });

    it('should match segments (pattern with matrix params)', () => {
      const matcher = new UrlSegmentMatcher(segments(['a', 'b', 'c', {param1: 'value1', param2: 'value2'}]), {matchMatrixParams: true, matchWildcardPathSegment: false});
  ...
  describe('Flags: {matchWildcardPathSegment: false}', () => {
    it('should match segments', () => {
      const matcher = new UrlSegmentMatcher(segments(['a', '*', 'c']), {matchMatrixParams: false, matchWildcardPathSegment: false});
      expect(matcher.matches(segments(['a', '*', 'c']))).toBeTrue();
      expect(matcher.matches(segments(['a', 'x', 'c']))).toBeFalse();
    });
  });

  describe('Flags: {matchWildcardPathSegment: true}', () => {
    it('should match segments', () => {
      const matcher = new UrlSegmentMatcher(segments(['a', '*', 'c']), {matchMatrixParams: false, matchWildcardPathSegment: true});
      expect(matcher.matches(segments(['a', '*', 'c']))).toBeTrue();
