  ...
  toHaveComponentState: (): CustomMatcher => {
    return {
      compare(viewId: ViewId, expected: string, failOutput: string | undefined): CustomMatcherResult {
        const nativeElement = TestBed.inject(WORKBENCH_VIEW_REGISTRY).get(viewId).portal.componentRef.location.nativeElement as HTMLElement;
        const actual = (nativeElement.querySelector('input.component-state') as HTMLOptionElement).value; // eslint-disable-line @typescript-eslint/non-nullable-type-assertion-style
        if (actual !== expected) {
