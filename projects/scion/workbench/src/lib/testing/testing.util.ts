...
export function styleFixture<T>(fixture: ComponentFixture<T>): ComponentFixture<T> {
  ...
  nativeElement.style.height = '500px';
  nativeElement.style.background = 'lightgray';
  return fixture;
}
...
export function clickElement(appFixture: ComponentFixture<any>, viewType: Type<any>, elementSelector: string, failureMessage?: string): void {
  ...
  const viewDebugElement = appFixture.debugElement.query(By.directive(viewType)) as DebugElement | undefined;
  if (!viewDebugElement) {
    throw Error(`View not showing [${viewType.name}]${failSuffix}`);
  }

  const linkDebugElement = viewDebugElement.query(By.css(elementSelector)) as DebugElement | undefined;
  if (!linkDebugElement) {
