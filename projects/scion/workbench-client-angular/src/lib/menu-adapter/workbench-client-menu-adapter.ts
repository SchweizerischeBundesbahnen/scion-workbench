...
export class WorkbenchClientMenuAdapter implements SciMenuAdapter {
  ...
  public menuContributions(location: `menu:${string}` | `toolbar:${string}` | `group:${string}`, context: Map<string, unknown>): Signal<SciMenuContributions> {
    if (!this._contributions.has(location)) {
      const contributions$ = this._workbenchMenuService.menuContributions(location, context).pipe(tap(contributions => console.log(`>>> contributions location=${location}`, contributions)), mapArray(contribution => {
        switch (contribution.type) {
          case 'menu': {
            return {
              type: 'menu',
  ...
  return [
    {
      ...
      useClass: WorkbenchClientMenuAdapter,
    },
  ];
}
