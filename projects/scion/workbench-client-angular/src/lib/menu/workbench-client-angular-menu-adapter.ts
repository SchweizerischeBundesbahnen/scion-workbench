...
import {ɵinjectMenuInjectionContextProviders, SciMenuAdapter, SciMenubarContributionLocation, SciMenubarFactoryFn, SciMenuContributionLocation, SciMenuContributionLocationLike, SciMenuContributionOptions, SciMenuFactoryFn, SciMenuFactoryFnLike, SciMenuFilterConfig, SciMenuItemLike, SciMenuOptions, SciMenuOrigin, SciMenuRef, SciToolbarContributionLocation, SciToolbarFactoryFn} from '@scion/components/menu';
import {Disposable, RequireOne} from '@scion/toolkit/types';
...
import {WorkbenchMenubarFactory, WorkbenchMenuFactory, WorkbenchMenuFilterConfig, WorkbenchMenuOrigin, WorkbenchToolbarFactory, ɵWorkbenchMenuService} from '@scion/workbench-client';
...
import {assertInInjectionContext, createDestroyableInjector, toLazyObservable} from '@scion/components/common';
...
export class WorkbenchClientAngularMenuAdapter implements SciMenuAdapter {
  ...
  public contributeMenu(location: SciMenuContributionLocationLike, factoryFn: SciMenuFactoryFnLike, options: SciMenuContributionOptions): Disposable {
    ...
    switch (parseMenuLocationType(location.location)) {
      case 'menu': {
        return this._workbenchMenuService.contributeMenu(location as SciMenuContributionLocation, (menu, context) => {
          ...
          const menuEnvironmentInjectionContextProviders = ɵinjectMenuInjectionContextProviders(context, {injector: this._injector});
          const menuEnvironmentInjectionContext = createDestroyableInjector({parent: this._injector, providers: menuEnvironmentInjectionContextProviders});

          runInInjectionContext(menuEnvironmentInjectionContext, () => tracked(() => menuFactoryFn(new WorkbenchClientMenuFactoryDelegate(menu), context), menu));
      ...
      case 'toolbar': {
        return this._workbenchMenuService.contributeMenu(location as SciToolbarContributionLocation, (toolbar, context) => {
          ...
          runInInjectionContext(menuEnvironmentInjectionContext, () => tracked(() => toolbarFactoryFn(new WorkbenchClientToolbarFactoryDelegate(toolbar), context), toolbar));
        }, {requiredContext: options.requiredContext, metadata: options.metadata, contributionInstant: options.contributionInstant});
      }
      case 'menubar': {
        return this._workbenchMenuService.contributeMenu(location as SciMenubarContributionLocation, (menubar, context) => {
          ...
          runInInjectionContext(menuEnvironmentInjectionContext, () => tracked(() => menubarFactoryFn(new WorkbenchClientMenubarFactoryDelegate(menubar), context), menubar));
        }, {requiredContext: options.requiredContext, metadata: options.metadata, contributionInstant: options.contributionInstant});
      }
    }
  ...
  public openMenu(menuLike: `menu:${string}` | SciMenuItemLike[], options: SciMenuOptions): SciMenuRef {
    ...
    const menuRef = this._workbenchMenuService.open(menu, {
      ...
      filter: filter && {
        ...
        notFoundMessage: toLazyObservable(filter.notFoundMessage, {injector: this._injector}),
        focus: filter.focus,
  ...
  public closeMenus(): void {
    this._workbenchMenuService.closeAll();
  }
}
...
function coerceFilterConfig(filter: SciMenuOptions['filter'] | undefined): SciMenuFilterConfig | undefined {
...
function parseMenuLocationType(location: `menu:${string}` | `toolbar:${string}` | `menubar:${string}`): 'menu' | 'toolbar' | 'menubar' {
  const regex = /^(?<type>(menu|toolbar|menubar)):(?<name>.+)$/;
  return regex.exec(location)!.groups!['type'] as 'menu' | 'toolbar' | 'menubar';
}
