...
export function toLazyObservable<T>(signal: MaybeSignal<T> | undefined, options?: {injector?: Injector}): Observable<NonNullable<T>> | undefined;
export function toLazyObservable<T>(signal: MaybeSignal<T> | undefined, options?: {injector?: Injector}): Observable<T> | undefined {
  if (signal === undefined) {
    return undefined;
  }
  if (!isSignal(signal)) {
