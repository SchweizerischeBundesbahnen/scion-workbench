...
export class InspectMessageBoxComponentPO {
  ...
  public readonly msgbox: MessageBoxPO;

  constructor(appPO: AppPO, public cssClass: string) {
    this.msgbox = appPO.messagebox({cssClass: cssClass});
    this.locator = this.msgbox.locate('app-inspect-message-box');
  }
  ...
  public async enterReturnValue(returnValue: string): Promise<void> {
    await this.locator.locator('input.e2e-return-value').fill(returnValue);
  }
}
