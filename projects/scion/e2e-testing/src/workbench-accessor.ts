...
import {ViewId, WorkbenchPart, WorkbenchService, WorkbenchView} from '@scion/workbench';
...
export class WorkbenchAccessor {
  ...
  public views(): Promise<Array<Pick<WorkbenchView, 'id' | 'alternativeId'>>> {
    return this._page.evaluate(() => {
      const workbenchService = (window as unknown as Record<string, unknown>).__workbenchService as WorkbenchService;
      return workbenchService.views().map(view => ({id: view.id, alternativeId: view.alternativeId}));
    });
  }
