...
export class BulkNavigationTestPagePO implements MicrofrontendViewPagePO {
  ...
  public async clickNavigateNoAwait(qualifier: Qualifier, extras: NavigationExtras): Promise<void> {
    ...
    await this.enterViewCount(extras.count);
  ...
  private async enterViewCount(viewCount: number): Promise<void> {
...
type NavigationExtras = Pick<WorkbenchNavigationExtras, 'target' | 'cssClass'> & {
  count: number;
};
