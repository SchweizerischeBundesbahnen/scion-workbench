...
import {WorkbenchMessageBoxOptions} from '@scion/workbench-client';
...
export class MessageBoxOpenerPagePO implements MicrofrontendViewPagePO {
  ...
  public async open(content: string | Qualifier, options?: WorkbenchMessageBoxOptions): Promise<void> {
    ...
    if (typeof content === 'string') {
      await this.locator.locator('select.e2e-capability-provider').selectOption('built-in');
      await this.locator.locator('input.e2e-message').fill(content);
    }
    else {
      await this.locator.locator('select.e2e-capability-provider').selectOption('microfrontend');
