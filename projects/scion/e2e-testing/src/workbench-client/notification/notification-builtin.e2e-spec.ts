...
import {NotificationTextMessagePO} from '../page-object/notification-text-message.po';
...
test.describe('Workbench Notification', () => {
  test.describe('Legacy Notification API', () => {
    test('should show notification with specified text (text as argument)', async ({appPO, microfrontendNavigator}) => {
      ...
      const notificationPage = new NotificationTextMessagePO(notification);
    ...
    test('should throw when not passing required params', async ({appPO, microfrontendNavigator}) => {
      ...
      await microfrontendNavigator.registerCapability<WorkbenchNotificationCapability>('host', {
        type: 'notification',
        qualifier: {component: 'notification-with-params', app: 'host'},
        private: false,
        properties: {
          path: '',
        },
      ...
      // Register intention.
      await microfrontendNavigator.registerIntention('app1', {type: 'notification', qualifier: {component: 'notification-with-params', app: 'host'}});

      // Display the notification.
      const notificationOpenerPage = await microfrontendNavigator.openInNewTab(NotificationOpenerPagePO, 'app1');
      const notification = notificationOpenerPage.show({component: 'notification-with-params', app: 'host'}, {
        legacyAPI: {enabled: true, textAsConfig: true},
      });
  ...
  test('should replace notifications of the same group', async ({appPO, microfrontendNavigator}) => {
    ...
    const notificationPage1 = new NotificationTextMessagePO(appPO.notification({cssClass: 'testee-1'}));
    ...
    const notificationPage2 = new NotificationTextMessagePO(appPO.notification({cssClass: 'testee-2'}));
    ...
    const notificationPage3 = new NotificationTextMessagePO(appPO.notification({cssClass: 'testee-3'}));
    ...
    const notificationPage4 = new NotificationTextMessagePO(appPO.notification({cssClass: 'testee-4'}));
    await expectNotification(notificationPage3).toBeVisible();
