...
test.describe('Activity Layout Maximize', () => {
  ...
  test('should restore minimized activities', async ({appPO, workbenchNavigator}) => {
    ...
    // Expect memoized activities to be restored
    await expect(appPO.workbenchRoot).toEqualWorkbenchLayout({
      activityLayout: {
        toolbars: {
          leftTop: {
  ...
  test('should minimize activity (minimize button top-right)', async ({appPO, workbenchNavigator}) => {
    await appPO.navigateTo({microfrontendSupport: false, mainAreaInitialPartId: 'part.initial'});
  ...
  test('should display minimize button only in top-right part', async ({appPO, workbenchNavigator}) => {
    await appPO.navigateTo({microfrontendSupport: false, mainAreaInitialPartId: 'part.initial'});
    ...
    // Expect minimize button to only be in top-right part.
