...
test.describe('View Drag & Drop (Tabbar)', () => {
  ...
  test('should swap tabs', async ({appPO, workbenchNavigator}) => {
    ...
    await workbenchNavigator.createPerspective(layout => layout
      .addPart('part.part')
      .addView('view.101', {partId: 'part.part', activateView: true})
    ...
    await dragHandle1.drop();
    await expect.poll(() => appPO.part({partId: 'part.part'}).bar.viewTabBar.getViewIds()).toEqual(['view.102', 'view.101']);
  ...
  test('should not display the drag source when dragging it over its tabbar', async ({appPO, workbenchNavigator}) => {
    ...
    await expect.poll(() => appPO.part({partId: 'part.part'}).bar.viewTabBar.getViewIds({visible: true})).toEqual(['view.102']);
  ...
  test('should snap the drag image to the tabbar when dragging near it', async ({appPO, workbenchNavigator}) => {
    ...
    const tabbarBounds = await appPO.part({partId: 'part.part'}).bar.viewTabBar.getBoundingBox();
  ...
  test('should render drag image as tab when dragging over tabbar', async ({appPO, workbenchNavigator}) => {
    await appPO.navigateTo({microfrontendSupport: false});
    await appPO.setDesignToken('--sci-workbench-tab-height', '35px');
    ...
    await test.step('Dragging tab near tabbar', async () => {
      ...
      await expect(async () => {
        ...
        const expectedCloseButtonColor = await dragHandle.getCssPropertyValue('--sci-color-text');
        const closeButtonColor = await dragHandle.getCssPropertyValue('color', {target: 'close-button'});
        expect(closeButtonColor).toEqual(expectedCloseButtonColor);
      }).toPass();
    });
  ...
  test('should disable drag image when not dragging it over a drop zone', async ({appPO, workbenchNavigator}) => {
    await appPO.navigateTo({microfrontendSupport: false});
    ...
    // Drag tab out of the workbench.
    await test.step('Dragging tab out of the workbench', async () => {
      await dragHandle.dragTo({x: 0, y: 0});
      await expect(dragHandle.locator).toBeVisible();

      // Expect drag image to look like a disabled drag image.
      await expect(async () => {
        const expectedBorderColor = await dragHandle.getCssPropertyValue('--sci-workbench-tab-drag-border-color-disabled');
        const border = await dragHandle.getCssPropertyValue('border');
        expect(border).toEqual(`1px solid ${expectedBorderColor}`);

        const expectedCloseButtonColor = await dragHandle.getCssPropertyValue('--sci-workbench-tab-drag-text-color-disabled');
        const closeButtonColor = await dragHandle.getCssPropertyValue('color', {target: 'close-button'});
        expect(closeButtonColor).toEqual(expectedCloseButtonColor);
      }).toPass();
    });

    // Drag tab back into the workbench.
    await test.step('Dragging tab back into the workbench', async () => {
      await test.step('Dragging tab out of tabbar', async () => {
        const viewportBounds = appPO.viewportBoundingBox();
        await dragHandle.dragTo({x: viewportBounds.hcenter, y: viewportBounds.vcenter});
        await expect(dragHandle.locator).toBeVisible();
  ...
  test('should shift tabs to the side when dragging over a tabbar', async ({appPO, workbenchNavigator}) => {
    ...
    await test.step('Dragging tab to the right after mid of tab 1', async () => {
      await dragHandle.dragTo({deltaX: 2, deltaY: 0});
    ...
    await test.step('Dragging tab to the left after mid of tab 3', async () => {
      await dragHandle.dragTo({deltaX: await getHCenter(tab3) - await getLeft(dragHandle) + 1, deltaY: 0});
      await expect.poll(() => getLeft(tab1)).toBe(tab1Bounds.left);
      await expect.poll(() => getLeft(tab2)).toBe(tab2Bounds.left);
      await expect.poll(() => getLeft(tab3)).toBe(tab3Bounds.left);
    });
    ...
    await test.step('Dragging tab to the left after mid of tab 2', async () => {
      await dragHandle.dragTo({deltaX: await getHCenter(tab2) - await getLeft(dragHandle) + 1, deltaY: 0});
      await expect.poll(() => getLeft(tab1)).toBe(tab1Bounds.left);
      await expect.poll(() => getLeft(tab2)).toBe(tab2Bounds.left);
      await expect.poll(() => getLeft(tab3)).toBe(tab3Bounds.left + tabWidth);
    });
    ...
    await test.step('Dragging tab to the left after mid of tab 1', async () => {
      await dragHandle.dragTo({deltaX: await getHCenter(tab1) - await getLeft(dragHandle) + 1, deltaY: 0});
      await expect.poll(() => getLeft(tab1)).toBe(tab1Bounds.left);
      await expect.poll(() => getLeft(tab2)).toBe(tab2Bounds.left + tabWidth);
      await expect.poll(() => getLeft(tab3)).toBe(tab3Bounds.left + tabWidth);
    });
  ...
  test('should disable pointer events for a smoother drag and drop experience', async ({appPO, workbenchNavigator}) => {
    ...
    await expect.poll(() => appPO.part({partId: 'part.part'}).bar.viewTabBar.getViewportClientCssProperty('pointer-events')).toEqual('none');
  });
  ...
  test('should animate shifting tabs when leaving the tabbar', async ({appPO, consoleLogs, workbenchNavigator}) => {
    ...
    await workbenchNavigator.createPerspective(layout => layout
      ...
      .addView('view.102', {partId: 'part.part'})
      .addView('view.103', {partId: 'part.part'})
      .addView('view.104', {partId: 'part.part'}),
    );
  ...
  test('should activate tab to the right when dragging tab out of its tabbar', async ({appPO, workbenchNavigator}) => {
    ...
    await expect.poll(() => tab3.isActive()).toBe(true);
  });
