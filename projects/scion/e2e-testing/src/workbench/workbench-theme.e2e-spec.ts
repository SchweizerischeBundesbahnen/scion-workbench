...
test.describe('Workbench', () => {
  ...
  test('should provide light and dark theme', async ({appPO, workbenchNavigator}) => {
    ...
    await test.step('light theme', async () => {
      ...
      await expect(appPO.workbenchRoot).toHaveCSS('background-color', 'rgb(255, 255, 255)');
      await expect(appPO.workbenchRoot).toHaveCSS('color-scheme', 'light');
    });
    ...
    await test.step('dark theme', async () => {
      ...
      await expect(appPO.workbenchRoot).toHaveCSS('background-color', 'rgb(29, 29, 29)');
      await expect(appPO.workbenchRoot).toHaveCSS('color-scheme', 'dark');
    });
  });
});
