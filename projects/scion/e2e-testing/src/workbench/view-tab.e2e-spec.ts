...
test.describe('Workbench View Tab', () => {
  ...
  test('should have disabled close button if view is blocked', async ({appPO, workbenchNavigator}) => {
    ...
    await dialogOpenerPage.open('dialog-page', {modality: 'view', context: {viewId}, cssClass: 'testee'});
  ...
  test('should use full tab width to render the heading', async ({appPO, workbenchNavigator}) => {
    ...
    // Expect heading not to overlap close button.
    await expect(async () => {
  ...
  test('should have stable title y-position when marking view dirty', async ({appPO, workbenchNavigator}) => {
    await appPO.navigateTo({microfrontendSupport: false, mainAreaInitialPartId: 'part.initial'});
    ...
    await expect.poll(async () => fromRect(await viewTab.title.boundingBox()).bottom).toEqual(titleBounds.bottom);
  });

  test('should have stable dirty marker y-position when clearing/entering title', async ({appPO, workbenchNavigator}) => {
    await appPO.navigateTo({microfrontendSupport: false, mainAreaInitialPartId: 'part.initial'});
    ...
    await expect.poll(async () => fromRect(await viewTab.dirty.boundingBox()).bottom).toEqual(dirtyMarkerBounds.bottom);
  ...
  test('should have stable heading y-position when marking view dirty', async ({appPO, workbenchNavigator}) => {
    await appPO.navigateTo({microfrontendSupport: false, mainAreaInitialPartId: 'part.initial'});
    await appPO.setDesignToken('--sci-workbench-tab-height', '3.5rem');
    ...
    await expect.poll(async () => fromRect(await viewTab.heading.boundingBox()).bottom).toEqual(headingBounds.bottom);
  });

  test('should have stable heading y-position when clearing/entering title', async ({appPO, workbenchNavigator}) => {
    await appPO.navigateTo({microfrontendSupport: false, mainAreaInitialPartId: 'part.initial'});
    await appPO.setDesignToken('--sci-workbench-tab-height', '3.5rem');
