...
test.describe('Workbench Component', () => {
  ...
  test('should destroy the workbench component when unmounting it', async ({appPO, consoleLogs}) => {
    ...
    await appPO.header.clickSettingsMenuItem({cssClass: 'e2e-navigate-to-blank-page'});
  ...
  test('should not restart the workbench when re-mounting the workbench component', async ({appPO, consoleLogs}) => {
    ...
    await appPO.header.clickSettingsMenuItem({cssClass: 'e2e-navigate-to-workbench-page'});
  ...
  test('should destroy part actions when unmounting the workbench component', async ({appPO}) => {
    ...
    await expect(appPO.activePart({peripheral: false}).bar.action({cssClass: 'e2e-open-new-tab'}).locator).toBeVisible();
    ...
    await expect(appPO.activePart({peripheral: false}).bar.action({cssClass: 'e2e-open-new-tab'}).locator).toHaveCount(1);
  });
});
