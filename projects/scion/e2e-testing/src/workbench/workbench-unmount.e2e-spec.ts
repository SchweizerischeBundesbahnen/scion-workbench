...
test.describe('Workbench Component', () => {
  ...
  test('should destroy part actions when unmounting the workbench component', async ({appPO}) => {
    ...
    await expect(await appPO.activePart.action({cssClass: 'e2e-open-new-tab'}).isVisible()).toBe(true);
    ...
    await expect(await appPO.activePart.action({cssClass: 'e2e-open-new-tab'}).count()).toBe(1);
  });
});
