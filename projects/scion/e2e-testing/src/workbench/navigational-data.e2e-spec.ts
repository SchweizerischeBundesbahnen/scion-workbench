...
test.describe('Navigational Data', () => {
  ...
  test.describe('Part', () => {
    ...
    test('should pass data (WorkbenchLayout.navigatePart)', async ({appPO, workbenchNavigator}) => {
      await appPO.navigateTo({microfrontendSupport: false});
    ...
    test('should replace/discard data when navigating part', async ({appPO, workbenchNavigator}) => {
      await appPO.navigateTo({microfrontendSupport: false});
      ...
      await workbenchNavigator.modifyLayout(layout => layout
        .navigatePart('part.testee', ['test-part'], {data: {data2: 'data 2'}}),
      );

      await expect.poll(() => partPage.getNavigationData()).toEqual({data2: 'data 2'});

      // Navigate part again without data.
      await workbenchNavigator.modifyLayout(layout => layout
        .navigatePart('part.testee', ['test-part']),
      );

      await expect.poll(() => partPage.getNavigationData()).toEqual({});

      // Navigate part again with different data.
      await workbenchNavigator.modifyLayout(layout => layout
        .navigatePart('part.testee', ['test-part'], {data: {data3: 'data 3'}}),
      );

      await expect.poll(() => partPage.getNavigationData()).toEqual({data3: 'data 3'});
    });
