...
export class CreatePerspectivePagePO {
  ...
  public async createPerspective(id: string, definition: PerspectiveDefinition): Promise<void> {
    ...
    // Enter the layout.
    const {dockedParts, parts, views, partNavigations, viewNavigations, activeParts, activeViews} = await definition.layout(new ɵWorkbenchLayoutFactory()) as ɵWorkbenchLayout;
    ...
    await LayoutPages.enterActiveViews(this.locator.locator('app-multi-value-input.e2e-active-views'), activeViews);
