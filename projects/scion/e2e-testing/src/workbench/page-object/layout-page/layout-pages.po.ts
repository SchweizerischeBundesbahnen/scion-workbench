...
export const LayoutPages = {
  ...
  enterActiveParts: async (locator: Locator, parts: string[] = []): Promise<void> => {
    await locator.locator('input').fill(parts.join(' '));
  },
  ...
  enterActiveViews: async (locator: Locator, views: string[] = []): Promise<void> => {
    await locator.locator('input').fill(views.join(' '));
  },

  /**
   * Enters parts to remove into {@link ModifyLayoutPageComponent} and {@link CreatePerspectivePageComponent}.
   */
