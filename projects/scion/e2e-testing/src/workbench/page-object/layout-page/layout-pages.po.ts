...
export const LayoutPages = {
  ...
  enterDockedParts: async (locator: Locator, dockedParts: DockedPartDescriptor[]): Promise<void> => {
    for (const [i, dockedPart] of dockedParts.entries()) {
      ...
      await new AppPO(locator.page()).waitUntilAngularStable();
      ...
      await locator.locator('input.e2e-class').nth(i).fill(coerceArray(dockedPart.cssClass).join(' '));
  ...
  enterParts: async (locator: Locator, parts: PartDescriptor[]): Promise<void> => {
    for (const [i, part] of parts.entries()) {
      ...
      await locator.locator('input.e2e-class').nth(i).fill(coerceArray(part.cssClass).join(' '));
    }
  },
  ...
  enterActiveParts: async (locator: Locator, parts: string[] = []): Promise<void> => {
    await locator.locator('input.e2e-active-parts').fill(parts.join(' '));
  },
  ...
  enterActiveViews: async (locator: Locator, views: string[] = []): Promise<void> => {
    await locator.locator('input.e2e-active-views').fill(views.join(' '));
  },
  ...
  enterRemoveParts: async (locator: Locator, parts: string[] = []): Promise<void> => {
    await locator.locator('input.e2e-remove-parts').fill(parts.join(' '));
  },
} as const;
