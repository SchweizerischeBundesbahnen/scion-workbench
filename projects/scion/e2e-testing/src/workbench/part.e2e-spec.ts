...
test.describe('Workbench Part', () => {
  ...
  test('should preserve title after history back and forward', async ({appPO, workbenchNavigator}) => {
    ...
    await expect(appPO.part({partId: 'part.right'}).bar.title).toHaveText('Part Title');

    // Enter title.
    await partPage.enterTitle('A');
    await expect(appPO.part({partId: 'part.right'}).bar.title).toHaveText('A');

    // Perform navigation back.
    await appPO.navigateBack();

    // Expect part not to be present.
    await expectPart(partPage.part).not.toBeAttached();
    ...
    await expect(appPO.part({partId: 'part.right'}).bar.title).toHaveText('B');
  });
  ...
  test('should detach parts of inactive activities', async ({appPO, workbenchNavigator}) => {
    await appPO.navigateTo({microfrontendSupport: false});
  ...
  test('should detach part on layout change', async ({appPO, workbenchNavigator}) => {
    ...
    // Add part to the left, forcing detaching the right part.
    await workbenchNavigator.modifyLayout(layout => layout
      .addPart('part.left', {align: 'left'})
      .navigatePart('part.left', ['test-part']),
    );
