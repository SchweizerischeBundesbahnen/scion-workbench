...
test.describe('Main Area', () => {
  ...
  test('should share main area between perspectives', async ({appPO, workbenchNavigator}) => {
    ...
    await expect.poll(() => appPO.view({viewId: 'view.1'}).tab.getInfo()).toMatchObject({viewId: 'view.1', urlSegments: 'path/to/view/1'} satisfies Partial<ViewInfo>);
    await expect.poll(() => appPO.view({viewId: 'view.2'}).tab.getInfo()).toMatchObject({viewId: 'view.2', urlSegments: 'path/to/view/2'} satisfies Partial<ViewInfo>);
    await expect.poll(() => appPO.view({viewId: 'view.3'}).tab.getInfo()).toMatchObject({viewId: 'view.3', urlSegments: 'path/to/view/3'} satisfies Partial<ViewInfo>);
