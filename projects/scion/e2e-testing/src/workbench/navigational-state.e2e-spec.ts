...
test.describe('Navigational State', () => {
  ...
  test.describe('View', () => {
    ...
    test('should maintain state when navigating back and forth in browser session history', async ({appPO, workbenchNavigator}) => {
      ...
      await expect.poll(() => appPO.workbench.view({id: 'view.2'}).then(view => view.navigation.state)).toEqual({state: 'a'});
      ...
      await expect.poll(() => appPO.workbench.view({id: 'view.2'}).then(view => view.navigation.state)).toEqual({state: 'b'});
      ...
      await expect.poll(() => appPO.workbench.view({id: 'view.2'}).then(view => view.navigation.state)).toEqual({state: 'c'});
  ...
  test.describe('Part', () => {
    ...
    test('should maintain state when navigating back and forth in browser session history', async ({appPO, workbenchNavigator}) => {
      ...
      await expect.poll(() => appPO.workbench.part({id: 'part.right'}).then(part => part.navigation.state)).toBeUndefined();

      // Navigate part with state 'a'.
      await layoutPage.modifyLayout(layout => layout.navigatePart('part.right', ['test-part'], {state: {state: 'a'}}));
      await expect.poll(() => appPO.workbench.part({id: 'part.right'}).then(part => part.navigation.state)).toEqual({state: 'a'});

      // Navigate part with state 'b'.
      await layoutPage.modifyLayout(layout => layout.navigatePart('part.right', ['test-part'], {state: {state: 'b'}}));
      await expect.poll(() => appPO.workbench.part({id: 'part.right'}).then(part => part.navigation.state)).toEqual({state: 'b'});

      // Navigate part with state 'c'.
      await layoutPage.modifyLayout(layout => layout.navigatePart('part.right', ['test-part'], {state: {state: 'c'}}));
      await expect.poll(() => appPO.workbench.part({id: 'part.right'}).then(part => part.navigation.state)).toEqual({state: 'c'});
