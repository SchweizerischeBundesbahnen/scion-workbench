...
test.describe('Default Page', () => {
  ...
  test('should display the default page when no view is opened', async ({appPO, workbenchNavigator}) => {
    ...
    await expect(await appPO.activePart({inMainArea: true}).isDefaultPagePresent('app-start-page')).toBe(true);
    ...
    await expect(await appPO.activePart({inMainArea: true}).isDefaultPagePresent('app-start-page')).toBe(false);
