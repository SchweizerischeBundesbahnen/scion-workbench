...
test.describe('Navigational CSS Classes', () => {
  ...
  test.describe('View', () => {
    ...
    test('should retain navigational CSS classes when moving view to new window', async ({appPO, workbenchNavigator}) => {
      ...
      const newViewPage = new ViewPagePO(newAppPO, {cssClass: 'testee-navigation'});
    ...
    test('should retain navigational CSS classes when moving view to other window', async ({appPO, workbenchNavigator}) => {
      ...
      const newViewPage1 = new ViewPagePO(newAppPO, {cssClass: 'testee-navigation-1'});
      ...
      const newViewPage2 = new ViewPagePO(newAppPO, {cssClass: 'testee-navigation-2'});
    ...
    test('should add CSS classes to inactive view (WorkbenchLayout.navigateView)', async ({appPO, workbenchNavigator}) => {
      ...
      await workbenchNavigator.modifyLayout(layout => layout
        .addView('view.100', {partId: 'part.initial', activateView: false})
  ...
  test.describe('Part', () => {
    ...
    test('should set CSS classes via part handle', async ({appPO, workbenchNavigator}) => {
      await appPO.navigateTo({microfrontendSupport: false});

      await workbenchNavigator.modifyLayout(layout => layout
        .addPart('part.testee', {align: 'left'})
      ...
      // Expect CSS classes of the route to be set.
      await expect.poll(() => partPage.part.getCssClasses()).toContain('e2e-test-part');

      await test.step('Updating CSS classes via part handle', async () => {
        await partPage.enterCssClass('testee-1');

        // Expect CSS classes set via handle to be set.
        await expect.poll(() => partPage.part.getCssClasses()).toContain('testee-1');
        // Expect CSS classes of the navigation to be set.
        await expect.poll(() => partPage.part.getCssClasses()).toContain('testee-navigation');
        // Expect CSS classes of the route to be set.
        await expect.poll(() => partPage.part.getCssClasses()).toContain('e2e-test-part');
      ...
      await test.step('Updating CSS classes via part handle', async () => {
        // Update CSS classes via part handle.
        await partPage.enterCssClass('testee-2');

        // Expect CSS classes previously set via handle not to be set.
        await expect.poll(() => partPage.part.getCssClasses()).not.toContain('testee-1');
        // Expect CSS classes set via handle to be set.
        await expect.poll(() => partPage.part.getCssClasses()).toContain('testee-2');
        // Expect CSS classes of the navigation to be set.
        await expect.poll(() => partPage.part.getCssClasses()).toContain('testee-navigation');
        // Expect CSS classes of the route to be set.
        await expect.poll(() => partPage.part.getCssClasses()).toContain('e2e-test-part');
