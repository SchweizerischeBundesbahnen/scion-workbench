...
test.describe('Activity Layout', () => {
  ...
  test('should show hint if activity is empty (has no navigated parts or views)', async ({appPO, workbenchNavigator}) => {
    await appPO.navigateTo({microfrontendSupport: false});
    ...
    await expect(activityPart.nullPartMessage).toBeVisible();
  });

  test('should show hint if both activities in panel are empty (have no navigated parts or views)', async ({appPO, workbenchNavigator}) => {
    await appPO.navigateTo({microfrontendSupport: false});
    ...
    await expect(activityPart1.nullPartMessage).toBeVisible();

    // Expect hint to show for activity.2.
    await expect(activityPart2.nullPartMessage).toBeVisible();
  });
