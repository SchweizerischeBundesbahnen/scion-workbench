...
test.describe('Workbench Layout Migration', () => {
  ...
  test('should migrate workbench layout v2 to the latest version', async ({appPO}) => {
    ...
    const [view3] = views.filter(view => view.partId === _33b22f60Part.id);
    if (!view3) {
      throw Error(`Migrated 'view.3' not found.`);
    }
  ...
  test('should migrate workbench layout v4 to the latest version', async ({appPO}) => {
    ...
    const views = await appPO.workbench.views();
    const [view2, view3] = views.filter(view => view.partId === leftPart.id);
    const [view4] = views.filter(view => view.partId === rightPart.id);
    if (!view2) {
      throw Error(`Migrated 'view.2' not found.`);
    }
    if (!view3) {
      throw Error(`Migrated 'view.3' not found.`);
    }
    if (!view4) {
      throw Error(`Migrated 'view.4' not found.`);
    }
