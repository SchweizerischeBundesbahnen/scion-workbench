...
async function assertPartGridElement(expectedPart: MPart, gridElementLocator: Locator, expectedGrids: ExpectedWorkbenchGrids): Promise<void> {
  ...
  if (expectedPart.activeViewId) {
    ...
    await throwIfAbsent(viewTabLocator, () => Error(`[DOMAssertError] Expected view tab 'wb-view-slot[data-viewid="${expectedPart.activeViewId}"]' to be active, but is not. [MPart=${JSON.stringify(expectedPart)}, locator=${viewTabLocator}]`));
  }
