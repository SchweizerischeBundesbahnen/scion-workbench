...
export class InspectMessageBoxPO {
  ...
  public readonly messageBoxPO: MessageBoxPO;

  constructor(public cssClass: string) {
    this.messageBoxPO = this._appPO.findMessageBox({cssClass: cssClass});
    this._pageFinder = this.messageBoxPO.$('app-inspect-message-box');
  }

  public async isDisplayed(): Promise<boolean> {
    await WebdriverExecutionContexts.switchToDefault();
    return await this.messageBoxPO.isDisplayed() && await this._pageFinder.isDisplayed();
  }
