...
describe('Workbench Router', () => {
  ...
  it('should allow navigating to a new view', async () => {
    await appPO.navigateTo({microfrontendSupport: false});
    ...
    await expect(await testeeViewTabPO.getViewId()).not.toEqual(routerViewId);
    await expect(await appPO.getViewTabCount()).toEqual(2);
  ...
  it('should allow navigating within the same view', async () => {
    await appPO.navigateTo({microfrontendSupport: false});
    ...
    await expect(await testeeViewTabPO.getViewId()).toEqual(routerViewId);
    await expect(await appPO.getViewTabCount()).toEqual(1);
  ...
  it('should, by default, activate a present view ', async () => {
    await appPO.navigateTo({microfrontendSupport: false});
    ...
    // activate the test view via routing
    await routerPagePO.enterPath('test-view');
    await routerPagePO.selectTarget('blank');
    await routerPagePO.enterMatrixParams({cssClass: 'testee'});
    await routerPagePO.clickNavigateViaRouter();

    // expect the test view to be activated and no new view to be opened
    await expect(await appPO.getViewTabCount()).toEqual(2);
    await expect(await testeeViewTabPO.isActive()).toBe(true);
