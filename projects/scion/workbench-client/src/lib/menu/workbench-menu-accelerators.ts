...
export function installMenuAccelerators(location: `menu:${string}`, options?: WorkbenchMenuAcceleratorOptions): Disposable {
  ...
  const subscription = Beans.get(ɵWorkbenchMenuService).menuItems$(location, context, {metadata: options?.metadata})
    ...
    .subscribe(({event, menuItems}) => {
      ...
      const matchingMenuItems = menuItems.filter(menuItem => matchesAccelerator(event, menuItem.accelerator!));
 ...
 */
function matchesAccelerator(event: KeyboardEvent, accelerator: WorkbenchKeyboardAccelerator): boolean {
 ...
 * enabling users to trigger actions without using a pointer device.
 */
