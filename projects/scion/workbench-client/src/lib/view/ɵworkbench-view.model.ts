...
export class ɵWorkbenchView implements WorkbenchView, PreDestroy {
  ...
  constructor(public id: ViewId) {
    void MicrofrontendPlatform.whenState(PlatformState.Stopping).then(() => {
      console.log('>>>> client:onPlatformStopping: sending stopping view');
  ...
  public preDestroy(): void {
    console.log('>>>> preDestroy view');
    void Promise.resolve().then(() => void Beans.get(MessageClient).publish('test/view', 'preDestroy view'));
    this._canCloseSubscription?.unsubscribe();
    this._canCloseSubscription = undefined;
    this._destroy$.next();
  }
