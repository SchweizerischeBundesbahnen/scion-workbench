...
export interface WorkbenchPartCapability extends Capability {
   ...
   *
   * Parameters can be read in the microfrontend by injecting the {@link WorkbenchPart} handle (or `ActivatedMicrofrontend` if a host microfrontend).
   * Parameters can also be referenced in the path, title, label, tooltip and resolvers using the colon syntax.
   *
   * @inheritDoc
   */
