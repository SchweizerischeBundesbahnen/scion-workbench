...
export interface WorkbenchPerspectiveCapability extends Capability {
  ...
  properties: {
     ...
     * area for user interaction. Views in the main area are retained when switching perspectives. A perspective cannot contribute
     ...
     */
    layout: [
      Pick<WorkbenchPerspectivePart, 'id' | 'views'>,
      ...WorkbenchPerspectivePart[]
    ];
...
export interface WorkbenchPerspectivePart {
  ...
  ratio?: number;
  /**
   * Defines views to add to the part. Views are added in declaration order. Views cannot be added to the main area part.
   *
   * A microfrontend provided as a view capability can be added as view. The qualifier differentiates between different
   * view capabilities. An application can reference the public view capabilities of other applications if it manifests
   * a respective intention.
   */
